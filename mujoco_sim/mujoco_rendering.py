...
class WindowViewer(BaseRender):
    ...
    def __init__(
        ...
        extra_views_camera_ids: Optional[List[int]] = None,
        max_geom: int = 1000,
        visual_options: Dict[int, bool] = {},
    ):
        glfw.init()
        ...
        self._render_extra_views = extra_views_camera_ids
        ...
        # Placeholder for external key callback
        self._external_key_callback = None
        super().__init__(model, data, width, height, max_geom, visual_options)
        glfw.swap_interval(1)

    def set_external_key_callback(self, callback):
        self._external_key_callback = callback
    ...
    def render(self):
        ...
        def update():
            ...
            # Only create extra camera views when visualization is enabled
            if self._render_extra_views:
                top_right_x = self.viewport.width - self.offscreen_width
                for i, cam_id in enumerate(self._render_extra_views[:4]):
                    offscreen_viewport = mujoco.MjrRect(
                        int(top_right_x), int(i * self.offscreen_height),
                    ...
                    mujoco.mjv_updateScene(
                        ...
                        offscreen_cam, mujoco.mjtCatBit.mjCAT_ALL.value, self.scn
                    )

                    mujoco.mjr_render(offscreen_viewport, self.scn, self.con)
    ...
    def _key_callback(self, window, key: int, scancode, action: int, mods):
        if self._external_key_callback is not None:
            self._external_key_callback(window, key, scancode, action, mods)
        ...
        elif key == glfw.KEY_A and self._paused is not None:
    ...
    def _create_overlay(self):
        ...
        if self._paused is not None:
            if not self._paused:
                ...
                self.add_overlay(
                    topleft, "Advance simulation by one step", "[A]"
                )
        self.add_overlay(
...
class MujocoRenderer:
    ...
    def __init__(
        ...
        self.extra_views_camera_ids = extra_views_camera_ids
        self._viewers = {}
        self.viewer = None
